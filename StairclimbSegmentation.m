    ...
    for k = 1:4 %BagType
        for j=1:2 %Task: 1=walk and turn, 2 = rescue climb
            if j == 1
                ...
                title(sprintf('%s %s sacrum gyro',subject{i},bag{k},task{j}))
                axis tight
                turntimes = ginput(2);
                Subject = [Subject;subject{i}];
                Load = [Load; bag{k}];
                Task = [Task; 'WAT'];
                SegmentTimes = [SegmentTimes;startTimes(AnnotInd_RC(k)),turntimes(:,1)',endTimes(AnnotInd_RC(k))];
            ...
            elseif j == 2
                ...
                figure;
                plot(tS(tS>=startTimes(AnnotInd_RC(k))),gS(tS>=startTimes(AnnotInd_RC(k)),2),'r')
                hold on
                ...
                title(sprintf('%s %s sacrum gyro',subject{i},bag{k}))
                axis tight
                turntimes = ginput(2);
                Subject = [Subject;subject{i}];
                Load = [Load; bag{k}];
...
T = table(Subject,Load,Task,SegmentTimes);