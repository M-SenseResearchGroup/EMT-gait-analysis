...
% % %         fprintf('ZUPTs\n')
        ...
        cwt(gSfilt);
         ...
         %plot(tS,gSfilt)
         % hold on
%         [pkss,locss] = findpeaks(gSfilt,tS,'MinPeakHeight',50);
%         %plot(locss,pkss,'*k')
%         [pksh,locsh] = findpeaks(-gSfilt,tS,'MinPeakHeight',20);
%         %plot(locsh,-pksh,'*b')
%         %find hs and to
%         HS=[];
%         TO=[];
%         for kk=2:length(locss)
%             if locss(kk)-locss(kk-1)<1.75
...
%          plot(tS,wavt(45,:));
%          hold on
%          plot(HS,ones(length(HS),1),'*m')        
%          plot(TO,ones(length(TO),1),'*g')      
     end
 end
...
% % % WATHB = sqrt(sum((pos_n(1:2,WATHB_eind)-pos_n(1:2,WATHB_sind)).^2));
% % % 
...
% % % WATLB = sqrt(sum((pos_n(1:2,WATLB_eind)-pos_n(1:2,WATLB_sind)).^2));
% % % 
...
% % % WATHS = sqrt(sum((pos_n(1:2,WATHS_eind)-pos_n(1:2,WATHS_sind)).^2));
% % % 
...
% % % WATLS = sqrt(sum((pos_n(1:2,WATLS_eind)-pos_n(1:2,WATLS_sind)).^2));
% % % 
...
% % % RCHB = sqrt(sum((pos_n(1:2,RCHB_eind)-pos_n(1:2,RCHB_sind)).^2));
% % % 
...
% % % RCLB = sqrt(sum((pos_n(1:2,RCLB_eind)-pos_n(1:2,RCLB_sind)).^2));
% % % 
...
% % % RCHS = sqrt(sum((pos_n(1:2,RCHS_eind)-pos_n(1:2,RCHS_sind)).^2));
% % % 
...
% % % RCLS = sqrt(sum((pos_n(1:2,RCLS_eind)-pos_n(1:2,RCLS_sind)).^2));
% % % 
% % % 
...
% % %            
% % %             stairs=step_table.Type_Activity~='N'; 
