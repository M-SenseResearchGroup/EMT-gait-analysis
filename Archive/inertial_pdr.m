...
load('subject_data'); %gyro_bias = [0.0386  -0.0488   -0.00]';
i=5;
timestamp = subj(i).rc.hb.dfr.a(:,1)'; % Timestamps of measurements.
acc_s = subj(i).rc.hb.dfr.a(:,2:4)'; % Accelerations in sensor frame.
gyro_s = subj(i).rc.hb.dfr.g(:,2:4)'; % Rates of turn in sensor frame.
data_size = length(acc_s);
g = 1; % Gravity.

%mag_acc=sqrt((subj(i).rc.hb.dfr.a(:,2).^2)+(subj(i).rc.hb.dfr.a(:,3).^2)+(subj(i).rc.hb.dfr.a(:,4).^2));
%plot(mag_acc)
