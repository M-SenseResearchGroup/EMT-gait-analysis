%% Stair Climb Task
%Load Data
load('Lsubj_data (1).mat')%1min

i=18;%subject
zed_thresh=1.25;
min_stance_time = 0.03; %min seconds in stance 
swing_thresh = 2; 
trial_start_offset = 0;%100;%seconds
...
for bagtype=1:4
    ...
    %
    figure;
    plot(tsa(tsa>=(tsa(1)+trial_start_offset)),gsa(tsa>=(tsa(1)+trial_start_offset),2))
    hold on
    plot(tsa(tsa>=(tsa(1)+trial_start_offset)),gmagsa(tsa>=(tsa(1)+trial_start_offset)))
    RCtimes=ginput(8);
                ...
                if length(state_buf)==4
                    ...
                    if isequal(state_buf,[1,0,1,0]) % Step = Footstrike-Footstrike
                        ...
                        if max(w_temp(trans_temp(1):trans_temp(2))) > swing_thresh && ...
                            ...
                            if tF(start_ind)>=(subjL(i).(cell2mat(activity(activitytype))).(cell2mat(bag(bagtype))).dfr.a(1,1)/1000)+trial_start_offset
